...
/*var sain = document.getElementById('sainCheckbox')as HTMLInputElement;
...
sain .addEventListener('click', this.onClick.bind(this));
onclick(event) {
  console.log(event);
}
if (sain.checked == true) {
    gravite.disabled = true;
    maladie.disabled = true;
  } else if (sain.checked = false) {
...
addAvisDroite(value){
  ...
  value ={
    ...
    "commentaireDroite": this.droiteComm ,
    "sainDroite":value.sainDroite } 
